...
#Oblpotrebsoyuz
def shop_oblpotrebsoyuz(request):
    shop = Shop.objects.values_list('slug', flat=True).distinct()
    local()
    address_str = str([i for i in str(request.path).split('/') if i][0])
    areas = Area.objects.values_list('name', 'slug').distinct()
    for slug in shop:
        for name_a, slug_a in areas:
            if slug == address_str and slug == slug_a:
                ...
                page_number = request.GET.get('page')
                page_obj = paginator.get_page(page_number)
                return render(request, 'oblpotrebsoyuz/products.html',
                              {'category_product': category_product, 'products': products,
                               'page_obj': page_obj, 'name': name, 'local': local, 'address_str': address_str})


def sort_list_oblpotrebsoyuz(request, list):
    shop = Shop.objects.values_list('slug', flat=True).distinct()
    local()
    address_str = str([i for i in str(request.path).split('/') if i][0])
    areas = Area.objects.values_list('name', 'slug').distinct()
    for slug in shop:
        for name_a, slug_a in areas:
            if slug == address_str and slug == slug_a:
                ...
                page_number = request.GET.get('page')
                page_obj = paginator.get_page(page_number)
                return render(request, 'shop/list_oblpotrebsoyuz.html', {'category_product': category_product, 'products': products, 'page_obj': page_obj, 'name': name, 'local': local,
                                                                    'address_str': address_str})


def shop_oblpotrebsoyuz_grid(request):
    local()
    shop = Shop.objects.values_list('slug', flat=True).distinct()
    areas = Area.objects.values_list('name', 'slug').distinct()
    address_str = str([i for i in str(request.path).split('/') if i][0])
    for slug in shop:
        for name_a, slug_a in areas:
            if slug == address_str and slug == slug_a:
                ...
                page_number = request.GET.get('page')
                page_obj = paginator.get_page(page_number)
                return render(request, 'oblpotrebsoyuz/grid.html', {'product': product, 'category_product': category_product, 'page_obj': page_obj, 'local': local, 'name': name,
                                                               'address_str': address_str})

# View product
def shop_oblpotrebsoyuz_product(request, id):
    local()
    name = str([i for i in str(request.path).split('/') if i][0])
    address_str = name
    slug_name = eval(name)
...
def shop_oblpotrebsoyuz_career(reguest):
    local()
    users = User.objects.all()
    categories = Category.objects.order_by('number')
    return render(reguest, 'oblpotrebsoyuz/career.html', {'users': users, 'categories': categories, 'local': local})


def cart_oblpotrebsoyuz(request):
    shop = Shop.objects.values_list('name', 'ogrn', 'uraddress', 'times', 'days', 'slug')
    shops = Shop.objects.values_list('name', 'slug').distinct()
    areas = Area.objects.values_list('name', 'slug').distinct()
    local()
    ...
    for n, s, dp, dpm, days_numb in local_d:
        for name_a, slug_a in areas:
            if s == address_str and s == slug_a:
                ...
                return render(request, 'oblpotrebsoyuz/cart.html', {'category_product': category_product, 'shop': shop, 'sbp': sbp, 'shops': shops, 'local': local, 'local_d': local_d, 'name': name,
                                                               'address_str': address_str})


def cart_ok_oblpotrebsoyuz(request, ord):
    shops = Shop.objects.values_list('name', 'phone', 'times', 'uraddress', 'slug').distinct()
    order = orders.objects.get(id=ord)
    shop = Shop.objects.values_list('slug', flat=True).distinct()
    areas = Area.objects.values_list('name', 'slug').distinct()
    ...
    for slug in shop:
        for name_a, slug_a in areas:
            if slug == address_str and slug == slug_a:
                ...
                name_slug = eval(slug)
                category_product = dict_category_product(name_slug)
    return render(request, 'oblpotrebsoyuz/cart_ok.html', {'local': local, 'name': name, 'category_product': category_product, 'categories': categories, 'order': order, 'shops': shops,
                                                      'address_str': address_str})


def searchoblpotrebsoyuz(request):
    areas = Area.objects.values_list('name', 'slug').distinct()
    local_d = Locations.objects.values_list('name', 'slug', 'delivery_price', 'delivery_price_min', 'days_numb').distinct()
    address_str = str([i for i in str(request.path).split('/') if i][0])
    for n, s, dp, dpm, days_numb in local_d:
    ...
    if request.method == 'POST':
        ...
        if orders.objects.filter(name=request.POST['name']).exists() == False:
            alert['name'] = 'Мы не нашли заказ в нашей базе. Попробуйте использовать другие параметры поиска'
            return render(request, 'oblpotrebsoyuz/search_order.html', alert)
        if orders.objects.filter(phone=request.POST['phone']).exists() == False:
            alert['phone'] = 'Мы не нашли заказ в нашей базе. Попробуйте использовать другие параметры поиска'
            return render(request, 'oblpotrebsoyuz/search_order.html', alert)
        else:
            client = orders.objects.filter(name=name, phone=phone)
            return render(request, 'oblpotrebsoyuz/search_order.html', {'category_product': category_product, 'client': client, 'local': local, 'address_str': address_str})
    return render(request, 'oblpotrebsoyuz/index.html', {'category_product': category_product, 'categories': categories, 'local': local, 'address_str': address_str})
